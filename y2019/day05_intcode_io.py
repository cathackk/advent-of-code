...
def part_1(tape: Tape, system_id: int = 1) -> int:
    """
    ...
    **First**, you'll need to add **two new instructions**:
      ...
      - Opcode `3` takes a single integer as **input** and saves it to the position given by its
        only parameter. For example, the instruction `3,50` would take an input value and store it
        at address `50`.
    ...
    **Second**, you'll need to add support for **parameter modes**:
    ...
    a two-digit number based only on the ones and tens digit of the value, that is, the opcode is
    the rightmost two digits of the first value in an instruction. Parameter modes are single
    digits, one per parameter, read right-to-left from the opcode: the first parameter's mode is in
    the hundreds digit, the second parameter's mode is in the thousands digit, the third parameter's
    ...
    works like it did before - `99` is written to address `4`.
        ...
        >>> Machine([1002, 4, 3, 4, 33], debug=True).run_through()
        [1] > head=0
        [1] > op=1002
        [1] > instr=MULTIPLY([4], 3, [4])
        [1] >> [4] * 3 -> [4] (33 * 3 = 99)
        [2] > head=4
        [2] > op=99
        [2] > instr=HALT()
    ...
    **Finally**, some notes:
      ...
      - Integers can be negative: `1101,100,-1,4,0` is a valid program (find 100 + -1, store the
        ...
        >>> Machine([1101, 100, -1, 4, 0]).run_through()
        [1101, 100, -1, 4, 99]
    ...
    code does the program produce?**
        ...
        >>> part_1([3, 1, 101, -1, 1, 1, 4, 1, 104, 0, 104, 333, 99])
        part 1: diagnostic code for system ID=1 is 333
        333
    """
...
def part_2(tape: Tape, system_id: int = 5) -> int:
    """
    ...
    Instead, you'll need to use the TEST to extend the thermal radiators. Fortunately, the diagnos-
    tic program (your puzzle input) is already equipped for this. Unfortunately, your Intcode
    computer is not.
    ...
    Here's a larger example. This example program uses an input instruction to ask for a single
    ...
    system to test, **provide it `5`**, the ID for the ship's thermal radiator controller.
    This diagnostic test suite only outputs one number, the **diagnostic code**.

    **What is the diagnostic code for system ID 5?**

        >>> part_2([3, 1, 102, 100, 1, 1, 4, 1, 99])
        part 2: diagnostic code for system ID=5 is 500
        500
    """

    result = Machine(tape).as_function_scalar()(system_id)

    print(f"part 2: diagnostic code for system ID={system_id} is {result}")
    return result

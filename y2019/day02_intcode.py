...
def part_1(tape: Tape) -> int:
    ...
    **Intcode** programs like the gravity assist program it was working on; surely there are enough
    ...
    For example, suppose you have the following program:
        ...
        >>> example_tape = [1, 9, 10, 3, 2, 3, 11, 0, 99, 30, 40, 50]
    ...
    Stepping forward 4 more positions arrives at opcode `99`, halting the program.
        ...
        >>> final_memory = Machine(example_tape, debug=True).run_through()
        [1] > head=0
        [1] > op=1
        [1] > instr=ADD([9], [10], [3])
        [1] >> [9] + [10] -> [3] (30 + 40 = 70)
    ...
    Here are the initial and final states of a few more small programs:
        ...
        >>> Machine([1, 0, 0, 0, 99]).run_through()  # 1 + 1 = 2
        [2, 0, 0, 0, 99]
        >>> Machine([2, 3, 0, 3, 99]).run_through()  # 3 * 2 = 6
        [2, 3, 0, 6, 99]
        >>> Machine([2, 4, 4, 5, 99, 0]).run_through()  # 99 * 99 = 9801
        [2, 4, 4, 5, 99, 9801]
        >>> Machine([1, 1, 1, 4, 99, 5, 6, 0, 99]).run_through()
        [30, 1, 1, 4, 2, 5, 6, 0, 99]
    ...
    replace position `2` with the value `2`.
        ...
        >>> example_tape_2 = [1, 0, 0, 0, 99, 0, 0, 0, 0, 0, 0, 0, 300]
        >>> adjusted_tape(example_tape_2)
        [1, 12, 2, 0, 99, 0, 0, 0, 0, 0, 0, 0, 300]

    **What value is left at position `0` after the program halts?**

        >>> part_1(example_tape_2)
        part 1: value at memory address 0 is 302
        302
    """

    result = Machine(adjusted_tape(tape)).run_through()[0]

    print(f"part 1: value at memory address 0 is {result}")
    return result
...
def part_2(tape: Tape) -> int:
    ...
    **What is `100 * noun + verb`?** (For example, if `noun=12` and `verb=2`, the answer would be
    `1202`.)
        ...
        >>> example_tape = [2, 0, 0, 0, 99, 0, 880, 1000, 1120, 1356, 1467, 1615, 17581, 1922]
        >>> part_2(example_tape)
        part 2: noun=8, verb=12 -> 96
        96
    """

    noun, verb = find_noun_verb(tape)
    result = noun * verb

    print(f"part 2: noun={noun}, verb={verb} -> {result}")
    return result


def adjusted_tape(tape: Tape, noun: int = 12, verb: int = 2) -> Tape:
    return tape[:1] + [noun, verb] + tape[3:]


def find_noun_verb(tape: Tape, target_value: int = 19690720) -> Optional[tuple[int, int]]:
    return next(
        (noun, verb)
        for noun in tqdm(range(100), delay=1.0)
        for verb in range(100)
        if Machine(adjusted_tape(tape, noun, verb)).run_through()[0] == target_value
    )

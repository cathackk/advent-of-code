...
def part_1(tape: Tape) -> int:
    ...
    to reach Santa in time. To do this, you'll need to configure a series of amplifiers already
    ...
    setting amplifier `A` to phase setting `3`, amplifier `B` to setting `1`, `C` to `2`, `D` to
    `4`, and `E` to `0`. Then, you could determine the output signal that gets sent from amplifier
    `E` to the thrusters with the following steps:
      ...
      - Start the copy of the amplifier controller software that will run on amplifier `A`. At its
        ...
        instruction, provide it the input signal, `0`. After some calculations, it will use an
    ...
    Here are some example programs:
        ...
        ... ]
        >>> max_thruster_signal(example_tape_1)
        43210
        ...
        ... ]
        >>> max_thruster_signal(example_tape_2)
        54321
...
def part_2(tape: Tape) -> int:
    ...
    to produce the thrust you'll need. The Elves quickly talk you through rewiring the amplifiers
    ...
    amplifier `A`'s input. This creates the feedback loop: the signal will be sent through
    ...
    Here are some example programs:
        ...
        ... ]
        >>> max_thruster_signal_feedback_loop(example_tape_1)
        139629729
        ...
        ... ]
        >>> max_thruster_signal_feedback_loop(example_tape_2)
        18216

    Try every combination of the new phase settings on the amplifier feedback loop.
    **What is the highest signal that can be sent to the thrusters?**

        >>> part_2(example_tape_2)
        part 2: highest thruster signal using feedback loop is 18216
        18216
    """

    result = max_thruster_signal_feedback_loop(tape)

    print(f"part 2: highest thruster signal using feedback loop is {result}")
    return result


def max_thruster_signal(tape: Tape, phases_range: range = range(5)) -> int:
    amp = Machine(tape)

    def amp_func(signal: int, phase: int) -> int:
        return amp.as_function_scalar()(phase, signal)

    return max(
        reduce(amp_func, phases, 0)
        for phases in permutations(phases_range)
    )


def max_thruster_signal_feedback_loop(tape: Tape, phases_range: range = range(5, 10)) -> int:
    amps = [Machine(tape) for _ in phases_range]

    def run_loop(phases: list[int]) -> int:
        amp_funcs = [
            amp.as_function_scalar(init=[phase], restarting=False)
        ...
        while True:
            try:
                signal = reduce((lambda sig, amp_func: amp_func(sig)), amp_funcs, signal)
            except StopIteration:
                return signal

    return max(run_loop(phases) for phases in permutations(phases_range))
